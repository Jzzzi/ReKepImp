...
def objective(opt_vars,
    ...
    unnormalized_opt_vars = unnormalize_vars(opt_vars, bounds)
...
class PathSolver:
    ...
    def __init__(self, config, ik_solver = None, reset_joint_pos = None):
        '''
        Args:
            config: (dict) configuration of the path solver
            ik_solver: not needed for now
            reset_joint_pos: not needed for now
        '''
        self._config = config
    ...
    def _check_opt_result(self, opt_result, path_quat, debug_dict, og_bounds):
        ...
        if debug_dict['path_violation'] is not None:
            ...
            path_constraints_satisfied = all([violation <= self._config['constraint_tolerance'] for violation in path_violation])
    ...
    def solve(self,
        ...
        num_control_points = get_linear_interpolation_steps(start_pose, end_pose, self._config['opt_pos_step_size'], self._config['opt_rot_step_size'])
        ...
        bounds = [(b_min, b_max) for b_min, b_max in zip(self._config['bounds_min'], self._config['bounds_max'])] + \
                    [(-np.pi, np.pi) for _ in range(3)]
        bounds *= (num_control_points - 2)
        bounds = np.array(bounds, dtype=np.float64)
        bounds = [(-1, 1)] * len(bounds)
        num_vars = len(bounds)
        ...
        # initial guess as linear interpolation
            ...
            init_sol = normalize_vars(init_sol, bounds)
        ...
        opt_result = self._check_opt_result(opt_result, poses_quat, debug_dict, bounds)
